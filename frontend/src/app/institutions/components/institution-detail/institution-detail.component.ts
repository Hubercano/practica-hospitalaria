...
export class InstitutionDetailComponent implements OnInit {
  ...
  institution: InstitutionDetail | null = null;
  ...
  rejectingRequirement: RequirementValue | null = null;
  ...
  initReqForm(req: RequirementValue) {
    ...
    const form = this.fb.group({
      ...
      expiryDate: [req.expiryDate ? req.expiryDate.split('T')[0] : '', req.definition.requiresExpiryDate ? Validators.required : []]
    });
  ...
  canSaveRequirement(req: RequirementValue) {
    const form = this.reqForms.get(req.id);
    if (!form) {
      return false;
    }
  ...
  toggleEdit(reqId: string) {
    if (this.editingReqs.has(reqId)) {
      ...
      const req = this.institution?.requirements.find((item) => item.id === reqId);
      if (req) {
        this.initReqForm(req);
      }
      return;
  ...
  openHistory(req: RequirementValue) {
    if (!req.documentSlotId) {
      return;
    }
  ...
  approveRequirement(req: RequirementValue) {
    if (!req.documentSlotId) {
      return;
    }
  ...
  openRejectModal(req: RequirementValue) {
  ...
  canInstitutionEdit(req: RequirementValue) {
    return this.isInstitutionUser() && req.status !== 'APPROVED';
  }

  getDocumentStatus(req: RequirementValue): 'SIN_CARGAR' | 'PENDING' | 'APPROVED' | 'REJECTED' {
  ...
  getDocumentStatusLabel(req: RequirementValue) {
  ...
  getDocumentStatusClass(req: RequirementValue) {
    switch (this.getDocumentStatus(req)) {
      case 'APPROVED':
  ...
  private buildFileUploadRequest(req: RequirementValue, expiryDate?: string) {
